// app/api/wbos/[slug]/dashboard/stats/route.ts
...
export async function GET(req: NextRequest) {
  try {
    // Extract businessId from the URL slug or JWT token
    const url = new URL(req.url);
    const pathParts = url.pathname.split("/");
    const slugIndex = pathParts.indexOf("[slug]");
    let businessId: string | null = null;

    // Try to get businessId from JWT first
    businessId = await getBusinessId(req);
    
    console.log("🔍 Dashboard Stats API called");
    console.log("📋 BusinessId from JWT:", businessId);

    if (!businessId) {
      return NextResponse.json({ error: "Unauthorized - No businessId found" }, { status: 401 });
    }
    ...
    console.log("📊 Raw stats fetched:", { totalBuses, totalTrips, totalRevenue });
    ...
    console.log("🔧 Maintenance & Tyres:", { totalMaintenance, totalTyres });
    ...
    console.log("✅ Final stats:", { totalRevenue, totalExpenses, netProfit });
    ...
    recentBuses.forEach((b) => {
      recentActivity.push({
        ...
        href: `/wbos/${url.pathname.split("/")[2]}/buses/${b.id}`,
        time: b.createdAt,
    ...
    recentVouchers.forEach((v) => {
      recentActivity.push({
        ...
        href: `/wbos/${url.pathname.split("/")[2]}/buses/${v.busId}/vouchers/${v.id}`,
        time: v.voucherDate,
    ...
    recentMaintenance.forEach((m) => {
      recentActivity.push({
        ...
        href: `/wbos/${url.pathname.split("/")[2]}/buses/${m.busId}/maintenance`,
    ...
    recentCargo.forEach((c) => {
      recentActivity.push({
        ...
        href: `/wbos/${url.pathname.split("/")[2]}/cargo`,
        time: c.date,
