// app/wbos/sadaat/buses/[busId]/tyres/page.tsx
"use client";

import { useEffect, useState } from "react";
import { useParams } from "next/navigation";
import Link from "next/link";
import { ArrowLeft, Plus, CircleDot, Edit, Calendar } from "lucide-react";
  ...
  const fetchData = async () => {
    try {
      const [busRes, recordsRes] = await Promise.all([
        ...
        fetch(`/api/wbos/sadaat/buses/${busId}/tyres`),
      ]);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[400px] bg-white">
        <div className="text-gray-500">Loading tyre records...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 p-6 bg-white min-h-screen">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link
          href={`/wbos/sadaat/buses/${busId}/tyres/create`}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-purple-50 rounded-lg p-4 border border-purple-100">
          <p className="text-sm text-purple-600">Total Tyre Records</p>
          <p className="text-xl font-bold text-purple-700">{records.length}</p>
        </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        {records.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">No tyre records yet</p>
            <Link
              href={`/wbos/sadaat/buses/${busId}/tyres/create`}
              className="inline-block mt-3 text-blue-600 hover:text-blue-700 text-sm"
            >
              Add your first tyre record
            </Link>
          </div>
        ) : (
                ...
                {records.map((record) => (
                  <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 text-sm text-gray-900">
                      {new Date(record.tyreDate).toLocaleDateString()}
                    </td>
